...
use crate::view_mode::{
    self, ColorInterpretation, ColorInterpretationWorker, PanchromaticParams, RasterViewHandle,
    ViewMode,
};
...
impl RasterView {
    ...
    fn update_datasets(&mut self) -> Result<()> {
        if let Some(path) = &self.raster_path {
            ...
            self.view_mode = Some(ViewMode::Single(ColorInterpretation::Panchromatic(
                raster_handler,
